...
import { passwordEliminarValida, requireAuth, requirePermiso } from '../auth.js'
...
vinculacionClientesRouter.get(
  '/',
  requireAuth,
  requirePermiso('panel-solicitudes'),
  async (_req, res, next) => {
  try {
...
vinculacionClientesRouter.post('/', async (req, res, next) => {
  try {
    ...
    // Genera el siguiente consecutivo con formato VC000001.
